...
**B1, em-dash reproduction (2026-06-13, 2026-06-14, 2026-06-14).** A punctuation constraint existed but was not run at emit time, and templates inside `user-preferences.md` kept reproducing the banned character. Fix shape: source elimination. The 2026-06-14 structural fix swept every em-dash out of the file and verified with grep, so no template could emit one. Status [FIXED]. Verified this run: grep over `user-preferences.md` returns zero occurrences, holding six weeks. See the scope caveat below.
...
**Scope caveat on B1.** The 2026-06-14 fix swept `user-preferences.md` only, while the rule it enforces is global. Two em-dashes currently sit in this file: line 42, inside the 2026-07-06 log row, and one introduced 2026-07-28 in the first draft of the BUILD owed-fix entry (since rewritten). Neither is a logged miss and neither postdates the fix within the file the fix targeted, so B1 stays [FIXED] rather than [REGRESSED]. Flagged because it marks unswept surface outside the fix's scope, which is the same shape of gap the fix was created to close.
