...
- An anchor-provenance line and an abort instruction, both mandatory. State for each anchor whether it was verified against a current read of the target, or label it unverified and name what would confirm it. Quote anchors including their terminal characters: a table cell's closing pipe, trailing whitespace, a code-fence marker. Never build an anchor by editing the replacement text; read it out of the target, since an anchor derived from a draft carries the draft's post-edit values. Instruct the executor to abort and report on mismatch rather than approximate. The executor's refusal to approximate is the enforcement; Claude's pre-emit read is the first check, not the only one.
- A post-edit check the executor runs and reports as a fact, required whenever the edit changes a count, renames a term, or alters a convention that other statements depend on. The check must produce arithmetic or a count, never an assessment: parent totals against the sum of their parts, a grep for the retired term returning zero, a reconciliation of a summary against the record it summarizes. "I swept the passage" is not a check. The check must also be valid for its target: a grep for a retired term proves nothing in a file that preserves retracted text on purpose, since superseded rows, withdrawn entries, and in-place corrections all keep the old wording by design. There, grep the assertion rather than the phrase, or scope the check past the correction and withdrawn blocks. A check that cannot return its pass value for structural reasons is a defective check, not a failed edit. Where the edit moves an existing block rather than rewriting it, instruct the executor to locate it by a unique opening string and move it unchanged, so no body text is reproduced from a draft. A check's number is evidence only if its unit and its pass condition are both stated and both sound. Name the unit the count is in: lines are not occurrences, files are not matches, characters are not bytes, and a tool that counts one while the expected figure describes the other produces a false failure or a false pass. Name the state each figure was taken in: a count or measurement is valid only for the read that produced it, so a figure carried across an intervening edit is restated as stale rather than reused as the expected value. And verify that the pass condition can be satisfied by the edit it checks: a condition contradicted by the replacement text (requiring zero matches of a string the replacement itself introduces) is unsatisfiable by construction, and the executor reports it FAIL rather than reinterpreting it as met. Where a check cannot be made sound, replace it with one that can: prefer a count scoped to a named range over a whole-file count, and prefer a reported before-and-after pair taken by the executor's own read over an expected value supplied by the author. This restates, at the check-writing surface, the state-validity rule Gate 4 Part A's access-state and emitted-artifact clauses carry at the claim-making surface. Both are in force; neither defers to the other. Gate 4 governs whether a figure may be asserted at all, this bullet governs whether a figure may serve as a check's expected value. A check's search string is an anchor and is subject to the same rule: quote it from the replacement text character for character, never retype it from the concept it expresses. An author who writes the check while thinking in the idea ("the post-edit check requirement") and an executor who greps the literal the text contains ("post-edit-check") will diverge silently on hyphenation, casing, inflection, and pluralization, and the check fails while the edit is sound. Where a string form is uncertain, have the executor report the counts of both candidate forms rather than predicting one.
